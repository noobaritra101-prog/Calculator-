...
async def cauc_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await db.is_bot_admin(update.effective_user.id): return
...
🎉 Sᴏʟᴅ ᴛᴏ {buyer_link} ғᴏʀ {item['current_bid']:,.1f} {curr_disp}!"""
            ...
            try:
                if item.get('photo_id'): await context.bot.edit_message_caption(chat_id=AUCTION_CHANNEL_ID, message_id=item['channel_message_id'], caption=sold_channel_text, parse_mode=ParseMode.HTML)
                else: await context.bot.edit_message_text(chat_id=AUCTION_CHANNEL_ID, message_id=item['channel_message_id'], text=sold_channel_text, parse_mode=ParseMode.HTML)
...
<b>Thank you for using Shrane Auction System ⚡</b>"""
            ...
            try: await context.bot.send_message(item['bidder_id'], text=buyer_text, parse_mode=ParseMode.HTML)
            except Exception: pass
...
<b>Thank you for trading on Shrane Auction System ⚡</b>"""
            ...
            try: await context.bot.send_message(item['seller_id'], text=seller_text, parse_mode=ParseMode.HTML)
            except Exception: pass
            
        else:
            ended_text = f"❌ **AUCTION ENDED (No Bids)**\n\nItem: ｢ {item['name']} 」"
            try:
                if item.get('photo_id'): await context.bot.edit_message_caption(chat_id=AUCTION_CHANNEL_ID, message_id=item['channel_message_id'], caption=ended_text, parse_mode="Markdown")
                else: await context.bot.edit_message_text(chat_id=AUCTION_CHANNEL_ID, message_id=item['channel_message_id'], text=ended_text, parse_mode="Markdown")
            except Exception: pass
            try: await context.bot.send_message(item['seller_id'], f"⚠️ Your auction for ｢ {item['name']} 」 ended, but unfortunately received no bids.")
            except Exception: pass
...
async def cauc_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "auction" and new_status == "on": await db.set_setting("submissions", "on")
...
async def clear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("⚠️ YES, Proceed", callback_data="clear_step1", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("❌ Cancel", callback_data="clear_cancel", api_kwargs={"style": "primary"})]
    ])
    await update.message.reply_text("⚠️ **WARNING: DATABASE WIPE**\nYou are about to clear all active and pending items.\nHistorical profile stats will be safely permanently archived.\n\nAre you sure you want to proceed?", reply_markup=kb, parse_mode="Markdown")
...
async def clear_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "clear_cancel":
        await query.edit_message_text("✅ Clear operation cancelled. Data is safe.")
    elif data == "clear_step1":
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🚨 CONFIRM WIPE 🚨", callback_data="clear_step2", api_kwargs={"style": "danger"})],
            [InlineKeyboardButton("❌ Cancel", callback_data="clear_cancel", api_kwargs={"style": "primary"})]
        ])
        await query.edit_message_text("🚨 **FINAL WARNING!** 🚨\n\nThis action cannot be undone. All live items will be removed.\n\nPress Confirm to wipe data now.", reply_markup=kb, parse_mode="Markdown")
    elif data == "clear_step2":
        await db.archive_active_to_users()
        await db.clear_all_active()
        await db.clear_all_pending()
        await query.edit_message_text("✅ **Database Cleared Successfully!**\nAll items removed. User history was archived to profiles safely.", parse_mode="Markdown")
...
async def generate_dstats_page(page: str, user_name: str):
    ...
    if page in ["users", "refresh"]:
        sellers = len(set(i['seller_id'] for i in all_active))
        buyers = len(set(i['bidder_id'] for i in all_active if i.get('bidder_id')))
        text += f"━━━❖ 👥 Uꜱᴇʀ Oᴠᴇʀᴠɪᴇᴡ ❖━━━\n👤 Tᴏᴛᴀʟ Uꜱᴇʀꜱ: {len(all_users):,}\n🛍️ Tᴏᴛᴀʟ Sᴇʟʟᴇʀꜱ: {sellers:,}\n🏆 Tᴏᴛᴀʟ Bᴜʏᴇʀꜱ: {buyers:,}\n🚫 Bᴀɴɴᴇᴅ Uꜱᴇʀꜱ: 0\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        
    elif page == "items":
        coins, gems, nuggets = len([i for i in all_active if i['currency'] == 'Coins']), len([i for i in all_active if i['currency'] == 'Gems']), len([i for i in all_active if i['currency'] == 'Nuggets'])
        text += f"━━━❖ 📦 Iᴛᴇᴍ Sᴛᴀᴛꜱ ❖━━━\n📦 Tᴏᴛᴀʟ Iᴛᴇᴍꜱ Lɪꜱᴛᴇᴅ: {len(all_active):,}\n💰 Iᴛᴇᴍꜱ ɪɴ Cᴏɪɴꜱ: {coins:,}\n💎 Iᴛᴇᴍꜱ ɪɴ Gᴇᴍꜱ: {gems:,}\n🪙 Iᴛᴇᴍꜱ ɪɴ Nᴜɢɢᴇᴛꜱ: {nuggets:,}\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        coins_vol, gems_vol, nuggets_vol = sum(i['current_bid'] for i in all_active if i['currency'] == 'Coins'), sum(i['current_bid'] for i in all_active if i['currency'] == 'Gems'), sum(i['current_bid'] for i in all_active if i['currency'] == 'Nuggets')
        bids = sum(len(i.get('bid_history', [])) for i in all_active)
        text += f"━━━❖ 💰 Eᴄᴏɴᴏᴍʏ Oᴠᴇʀᴠɪᴇᴡ ❖━━━\n💰 Tᴏᴛᴀʟ Cᴏɪɴꜱ ɪɴ Cɪʀᴄᴜʟᴀᴛɪᴏɴ: {coins_vol:,.0f}\n💎 Tᴏᴛᴀʟ Gᴇᴍꜱ: {gems_vol:,.0f}\n🪙 Tᴏᴛᴀʟ Nᴜɢɢᴇᴛꜱ: {nuggets_vol:,.0f}\n💸 Tᴏᴛᴀʟ Tʀᴀɴꜱᴀᴄᴛɪᴏɴꜱ: {bids:,}\n━━━━━━━━━━━━━━━━━━━━━━━\n"
        
    elif page == "system":
        db_size_mb = await db.get_db_size_mb()
        db_used_percent = (db_size_mb / 500.0) * 100 if db_size_mb > 0 else 0.01
        text += f"━━━❖ ⚙️ Sʏꜱᴛᴇᴍ & Dᴀᴛᴀ ❖━━━\n🗂️ Dᴀᴛᴀʙᴀꜱᴇ Sɪᴢᴇ: {db_size_mb:.2f} MB\n📊 Dᴀᴛᴀʙᴀꜱᴇ Uꜱᴇᴅ: {db_used_percent:.2f}% / 500 MB\n⏳ Bᴏᴛ Rᴜɴ Tɪᴍᴇ: {days}ᴅ {hours}ʜ {minutes}ᴍ\n🔄 Lᴀᴛᴇɴᴄʏ: Pɪɴɢɪɴɢ PɢBᴏᴜɴᴄᴇʀ...\n━━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def dstats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except Exception: await query.answer("Already up to date.")
